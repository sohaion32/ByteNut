...
class BytenutRenewal:
    ...
    def poll_start_status(self, sb, server_id, timeout=300, interval=5):
        ...
        while time.time() < deadline:
            ...
            if data:
                ...
                self.log(f"  队列: inQueue={in_queue}, pos={pos}, wait={wait_sec}s, msg={msg}")
    ...
    def _wait_dialog_turnstile(self, sb, timeout=30):
        ...
        while time.time() - start < timeout:
            ...
            if sb.execute_script("""
                var btn = document.querySelector('div.el-dialog__footer button.el-button--primary');
            ...
            try:
                val = sb.execute_script("""
                    ...
                    if (!d) return '';
                    var i = d.querySelector('input[name="cf-turnstile-response"]');
                    return i ? i.value : '';
                """)
            ...
            if now - last_click > 3:
                try:
                    ...
                    try:
                        sb.execute_script("""
                            var d = document.querySelector('div.el-dialog');
                            if (d) { var ts = d.querySelector('.cf-turnstile'); if (ts) ts.click(); }
                        """)
                        last_click = now
                    except Exception:
                        pass
    ...
    def ui_start_server(self, sb, server_id, idx):
        ...
        try:
            ...
            try:
                sb.execute_script("""
                    document.querySelectorAll('.el-sub-menu__title span').forEach(function(el){
                        if (el.textContent.trim() === 'Management') el.closest('.el-sub-menu__title').click();
                    });
                """)
                time.sleep(2)
            except Exception as e:
        ...
        try:
            ...
            try:
                sb.execute_script("""
                    document.querySelectorAll('.el-menu-item span').forEach(function(el){
                        if (el.textContent.trim() === 'Console') el.closest('.el-menu-item').click();
                    });
                """)
                time.sleep(3)
            except Exception as e:
        ...
        for attempt in range(30):
            ...
            """):
                sb.execute_script("""
                    document.querySelector('div.el-dialog__footer button.el-button--primary').click();
                """)
        ...
        try:
            ...
            for _ in range(5):
                has_q = sb.execute_script(
                    "return !!document.querySelector('div.el-message-box.queue-dialog-styled');"
                )
                if has_q:
                    break
                time.sleep(1)
            if has_q:
                self.log("📋 排队弹窗，点击 OK...")
                sb.execute_script("""
                    document.querySelectorAll('div.el-message-box.queue-dialog-styled .el-message-box__btns button')
                    .forEach(function(btn){ if (btn.textContent.trim() === 'OK') btn.click(); });
                """)
                time.sleep(2)
                self.log("✅ 排队弹窗已关闭")
            else:
            ...
            with SB(
                ...
                try:
                    ...
                    try:
                        ...
                        sb.wait_for_element_visible('input[type="text"]', timeout=15)
                        sb.type('input[type="text"]', user)
                    try:
                    ...
                    if not server_id:
                        self.send_tg("❌", "失败", user, "未知", state, expiry_str, "服务器ID无效",
                    ...
                    self.log(f"可续期={can_extend}, 冷却={cooldown_min}分, 距过期={mins_until_exp}分")
                    ...
                    if state == "offline":
                        if can_extend:
                            ...
                            if not ready:
                                self.send_tg("❌", "面板加载失败", user, server_id, "offline", expiry_str,
                                             screenshot=self.shot(sb, f"panel_fail_{idx}.png"))
                                continue
                            if not self.click_renew_menu(sb, server_id, idx):
                                self.send_tg("❌", "续期菜单失败", user, server_id, "offline", expiry_str,
                            ...
                            if result is True:
                                if not self.wait_until_not_expired(sb, server_id):
                                    self.send_tg("⚠️", "续期成功但状态未更新", user, server_id, "offline", expiry_str,
                                                 "无法开机，请稍后重试",
                                                 screenshot=self.shot(sb, f"start_fail_{idx}.png"))
                                    continue
                                ok, final = self.ui_start_server(sb, server_id, idx)
                                self.send_tg("✅" if ok else "⚠️",
                                             "续期并开机成功" if ok else "续期成功，开机未确认",
                                             user, server_id, f"offline -> {final}",
                                             f"{expiry_str} -> {new_time}",
                                             screenshot=self.shot(sb, f"ok_{idx}.png"))
                            elif result == "cooldown":
                                self.send_tg("⏳", "续期后冷却", user, server_id, "offline", expiry_str,
                            ...
                            if expired:
                                self.send_tg("🚫", "无法操作", user, server_id, state, expiry_str,
                                             "服务器已过期且处于冷却期",
                                             screenshot=self.shot(sb, f"expired_cooldown_{idx}.png"))
                            else:
                                self.log("🔴 离线冷却中，直接开机（UI）")
                                ok, final = self.ui_start_server(sb, server_id, idx)
                                self.send_tg("✅" if ok else "❌",
                                             "开机成功" if ok else "开机失败",
                                             user, server_id, f"offline -> {final}", expiry_str,
                                             screenshot=self.shot(sb, f"{'started' if ok else 'start_fail'}_{idx}.png"))
                        continue
                    ...
                    ready = self.navigate_to_panel(sb, server_id)
                    if not ready:
                        self.send_tg("❌", "面板加载失败", user, server_id, state, expiry_str,
                                     screenshot=self.shot(sb, f"panel_fail_{idx}.png"))
                        continue
                    if not self.click_renew_menu(sb, server_id, idx):
                        self.send_tg("❌", "续期菜单失败", user, server_id, state, expiry_str,
                                     screenshot=self.shot(sb, f"renew_fail_{idx}.png"))
                        continue
                    result, new_time = self.try_extend_and_verify(sb, server_id, expired_time)
                    if result is True:
                        ...
                        self.send_tg("⏳", "续期后冷却", user, server_id, state, expiry_str,
                                     screenshot=self.shot(sb, f"cooldown_{idx}.png"))
                    else:
